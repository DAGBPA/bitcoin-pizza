 ...
 * Premine pubkeys
 */
std::vector<std::vector<std::string> > gPreminePubkeys = {
...
public:
    CMainParams() {
        ...
        consensus.BPAHeight = 501888; // Around 01/01/2018 00:00 UTC
        consensus.BPAPremineWindow = 16800;
        consensus.BPAPremineEnforceWhitelist = true;
        consensus.powLimit = uint256S("0007ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitStart = uint256S("00001fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        ...
        nDefaultPort = 8888; // different port than Bitcoin
        ...
        vSeeds.emplace_back("dnsseed.bitcoinpizza.cc", true);
        vSeeds.emplace_back("seed1.bitcoinpizza.cc", false);
        vSeeds.emplace_back("seed2.bitcoinpizza.cc", false);
        vSeeds.emplace_back("seed3.bitcoinpizza.cc", false);
        vSeeds.emplace_back("seed4.bitcoinpizza.cc", false);

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,55);  // prefix: P
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,80);  // prefix: Z
        ...
        vPreminePubkeys = gPreminePubkeys;
    }
};
...
public:
    CTestNetParams() {
        ...
        consensus.BPAHeight = 1256240;
        consensus.BPAPremineWindow = 500;
        consensus.BPAPremineEnforceWhitelist = true;
        consensus.powLimit = uint256S("0007ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitStart = uint256S("0006ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.powLimitLegacy = uint256S("00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        ...
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        vSeeds.emplace_back("test-dnsseed.bitcoinpizza.cc", true);
        vSeeds.emplace_back("test-seed1.bitcoinpizza.cc", false);
        vSeeds.emplace_back("test-seed2.bitcoinpizza.cc", false);
        vSeeds.emplace_back("test-seed3.bitcoinpizza.cc", false);
...
public:
    CRegTestParams() {
        ...
        consensus.BPAHeight = 3000;
        consensus.BPAPremineWindow = 10;
        consensus.BPAPremineEnforceWhitelist = true;
...
CScript CltvMultiSigScript(const std::vector<std::string>& pubkeys, uint32_t lock_time) {
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    assert((uint32_t)consensus.BPAHeight <= height &&
           height < (uint32_t)(consensus.BPAHeight + consensus.BPAPremineWindow));
    CScript scriptDummy = CScript() << OP_TRUE;
    if (!consensus.BPAPremineEnforceWhitelist || scriptPubKey == scriptDummy) {
        return true;
    }
    int block = height - consensus.BPAHeight;
