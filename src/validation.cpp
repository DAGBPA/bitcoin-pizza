...
const std::string strMessageMagic = "Bitcoin Pizza Signed Message:\n";
...
bool static IsBPAHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.BPAHeight;
}

bool IsBPAHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsBPAHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsBPAHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsBPAHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.BPAHeight;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsBPAHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.BPAHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after BPA fork.
    if (nHeight >= consensusParams.BPAHeight && block.nHeight != (uint32_t)nHeight)
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BPAHeight &&
        nHeight < consensusParams.BPAHeight + consensusParams.BPAPremineWindow &&
        consensusParams.BPAPremineEnforceWhitelist)
    {
        if (block.vtx[0]->vout.size() != 1) {
            if (!(block.vtx[0]->vout.size() == 2 && block.vtx[0]->vout[1].nValue == 0)) {
                return state.DoS(
