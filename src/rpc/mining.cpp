...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {
        CScript newscript = coinbaseScript->reserveScript;
        // Change coinbase script
        if (params.GetConsensus().BPAPremineEnforceWhitelist && nHeight+1 >= params.GetConsensus().BPAHeight &&
            nHeight+1 < params.GetConsensus().BPAHeight + params.GetConsensus().BPAPremineWindow) {
                int block = nHeight+1 - (uint32_t)params.GetConsensus().BPAHeight;
                const std::vector<std::string> pubkeys = params.vPreminePubkeys[block % params.vPreminePubkeys.size()];  // Round robin.
                ...
                newscript = target_scriptPubkey;
        }
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(newscript));
        ...
        if (pblock->nHeight < (uint32_t)params.GetConsensus().BPAHeight) {
            // Solve sha256d.
...
// NOTE: Unlike wallet RPC (which use BPA values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric, optional) estimate fee-per-kilobyte (in BPA)\n"
