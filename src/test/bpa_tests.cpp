...
#include "test/bpa_cltv_multisig_data.h"
...
BOOST_FIXTURE_TEST_SUITE(bpa_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(zcash_header_compatible)
{
    ...
    bpa_header.nHeight = 10000000;
    // `CBlockHeader.nReserved[5:7]` should be placed in the last 8 bytes of `ZcashBlockHeader.hashReserved`.
    bpa_header.nReserved[5] = 1234;
    bpa_header.nReserved[6] = 0;
    bpa_header.nSolution = std::vector<unsigned char> {
        0x11, 0x22, 0x33
    };
    ...
    BOOST_CHECK_EQUAL(bpa_header.nHeight, (uint32_t)zcash_header.hashReserved.GetUint64(0));
    BOOST_CHECK_EQUAL(bpa_header.nReserved[5], (uint32_t)zcash_header.hashReserved.GetUint64(3));
    BOOST_CHECK_EQUAL(bpa_header.nSolution[2], zcash_header.nSolution[2]);
}
